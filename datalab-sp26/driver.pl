...
while ($line = <INFILE>) {
    ...
    $total_c_score += $score_map{$c_rating};
    }
...
$tscore = $total_c_score + $total_p_score;

print "\nScore = $tpoints/$tscore [$total_c_points/$total_c_score Corr + $total_p_points/$total_p_score Perf] ($tops total operators)\n";
